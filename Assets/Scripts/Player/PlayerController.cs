   ...
   private void getState()
   {
    ...
    {   
        ...
        if ((currentHealth -= damage) <= 0)
        {
            ...
            SceneManager.LoadScene("gameOver-Lose");    // Change scene screen to the Lose screen

        }
        else
        {
            currentHealth -= damage;    // Decreases player's current health if the player's current health is not 0 or below
        }
